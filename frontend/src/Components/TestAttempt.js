...
function TestAttempt() {
  ...
  useEffect(() => {
    // Only set questions once when component mounts
    ...
    const timer = setInterval(() => {
      ...
      // Check if current time reaches or exceeds the end_time
      if (currentTime >= endTime || timeLeft <= 0) {
        handleSubmit(); // Submit automatically
        clearInterval(timer); // Stop the timer
      } else {
        setTimeLeft(prevTime => prevTime - 1); // Decrement the timer
      }
    }, 1000);
  ...
  const handleSubmit = async () => {
    ...
    state.end_time = endTime;  // Store end time in the state object
  
    // Get the start time from the state
    const [startHours, startMinutes] = state.start_time.split(':').map(Number);
    
    // Convert start time and end time to minutes for difference calculation
    ...
    // Extract minutes and seconds from the total time taken
    const minutes = Math.floor(timeTakenInMinutes);
    const seconds = current.getSeconds();
  
    // Format the time taken as mm:ss
    const formattedTimeSpent = `${String(minutes).padStart(2, '0')}:${String(seconds).padStart(2, '0')}`;
  
    // Loop through all questions
    questions.forEach((question, index) => {
      const selectedOption = selectedOptions[index]; // Get the selected option for the current question
  
      // Check if the question was answered
      if (selectedOption != null) {
        // Compare selected option with validity_answer
    ...
    finalObj.time_taken = formattedTimeSpent; // Add time taken in mm:ss format
    ...
    if(confirm){
      const res = await axiosWithToken.post(`${BASE_URL}/student-api/end-test`,finalObj)
      if(res.data.message=='Test Ended Successfully'){
        navigate(`../studentdashboard`,{state:{message:res.data.message}});
      }
    }
    ...
    !termsAccepted ? (
      <>
        ...
        <div className="terms-modal">
          ...
          <div className="terms-content">
            <p>Before you start the test, please read and accept the terms and conditions below:</p>
            <ul>
      ...
      <div className="quiz-page">
        <header className="quiz-header">
          <h1>EduAssess</h1>
          <button type="submit" className="text-light btn btn-success" onClick={handleSubmit}>
            Submit Test
          </button>
        ...
        <main className="quiz-main">
          <div className="question-section">
              ...
              {[1, 2, 3, 4].map((optionIndex) => (
                <button
                  ...
                  className={`option-button ${
                    selectedOptions[currentQuestion] === optionIndex ? 'selected' : ''
                  }`}
            ...
            <div className="navigation-buttons">
              ...
              <button className="save-next-button" onClick={handleSaveAndNextClick}>
                Save & Next
              </button>
              <button
