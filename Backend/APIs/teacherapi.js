...
teacherApp.put("/upgrade/:username/:plan_type", async (req, res) => {
  ...
  let init_user_sub = await subCollection.findOne({
    $and: [{ username: uname }, { active_status: true }],
  });
  ...
  if (init_user_sub != null) {
    let init_user = await subCollection.findOne(
      { username: uname },
      { sort: { subId: -1 } }
    );
...
teacherApp.get("/display-tests/:uname", async (req, res) => {
  ...
  const testobj = await testTeacher
    .find({ $and: [{ createdBy: username }] })
    .toArray();
  console.log(testobj);
  if (testobj.length != 0) {
...
teacherApp.put(
  "/modify-qs/:s_uname/:type",
  subscriptionVerify,
  async (req, res) => {
    const qsobj = req.body;
    //console.log(qsobj)
    const updated = await teacherQuestions.findOneAndUpdate(
      { $and: [{ qs_id: qsobj.qs_id }, { username: qsobj.username }] },
      { $set: { ...qsobj } },
...
teacherApp.put(
  ...
  async (req, res) => {
    ...
    console.log(cby);
    //console.log(qsobj)
    const updated = await testTeacher.findOneAndUpdate(
      { $and: [{ testid: testobj.testid }, { createdBy: cby }] },
      { $set: { ...testobj } },
      { returnOriginal: false }
    );
    console.log(updated);
...
teacherApp.put("/correct/:uname/:testid/:marks", async (req, res) => {
  ...
  const userobj = await testTeacher.findOne({
    $and: [{ createdBy: uname }, { testid: test_id }],
  });
  console.log(userobj);
  ...
  const updated = await testTeacher.findOneAndUpdate(
    { $and: [{ createdBy: uname }, { testid: test_id }] },
    { $set: { questions: userobj.questions } },
    { returnOriginal: false }
  );
  console.log(updated);
  if (!updated) {
...
teacherApp.put(
  "/activate/:uname/:testid/:s_uname/:type",
  subscriptionVerify,
  async (req, res) => {
    const uname = req.params.uname;
    const test_id = +req.params.testid;
    const testobj = await testTeacher.findOneAndUpdate(
      { $and: [{ createdBy: uname }, { testid: test_id }] },
      { $set: { validity: true } },
      { returnOriginal: false }
    );
...
//Generate a question
/*teacherApp.post("/generate-question", async (req, res) => {
  
  try {
    const { subject, sub_type } = req.body;
    const { pipeline } = await import("@xenova/transformers");
    // Load the question generation model
    const questionGenerator = await pipeline("text2text-generation", "Xenova/t5-small");

    const prompt = `Generate a question about ${subject} under the topic ${sub_type}`;
    
    // Generate question
    const output = await questionGenerator(prompt, { max_length: 100, num_return_sequences: 1 });

    res.json({ question: output[0].generated_text }); // Send back the question
  } catch (error) {
